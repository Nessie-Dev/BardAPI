class Bard {
  ...
  async #getImageUrl(url) {
      try {
          ...
          const autoLoginHeader = response.headers.get('x-auto-login');
          const match = autoLoginHeader.match(/continue%3D([^&]*)/);
  ...
  async #uploadImage(name, buffer) {
      ...
      try {
          ...
          throw new Error(
              "Could not fetch Google Bad. You may be disconnected from internet: " +
              e
          );
      }
  }
  ...
  async #query(message, config) {
      ...
      // HTTPS parameters
      const params = {
          bl: "boq_assistant-bard-web-server_20240228.09_p0",
          _reqID: ids?._reqID ?? "0",
          rt: "c",
      };
      ...
      let match_responses;
      while ((match_responses = regex_responses.exec(chatData)) !== null) {
          const response = match_responses[1];
      ...
      // Define the regex pattern to extract URLs
      const regex_url = /https:\/\/[^"]+/g;
      const img = [];
      let match_url;

      // Extract URLs from chatData
      while ((match_url = regex_url.exec(chatData)) !== null) {
          img.push(match_url[0]);
      }

      // Separate images and other URLs based on conditions
      const images = [];
      const urls = [];

      // Use a for loop for proper handling of async operations
      ...
      const regex_c_r = /\["(c_[\da-f]+)","(r_[\da-f]+)"\]/g;
      const c = [];
      const r = [];

      let match_c_r;
      while ((match_c_r = regex_c_r.exec(chatData)) !== null) {
          c.push(match_c_r[1]);
          r.push(match_c_r[2]);
      }

      const regex_rc = /"rc_([\da-f]+)"/g;
      const rc = [];

      let match_rc;
      while ((match_rc = regex_rc.exec(chatData)) !== null) {
          rc.push(`rc_${match_rc[1]}`);
      }
